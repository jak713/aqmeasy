...
class CsvController:
    ...
    def run_aqme(self):
        """runs aqme csearch in the background (new QThread when I understand how that works) and pipes the output to the shell_output window."""
        def run_aqme_thread():
            # Create a small stream that appends output to the shell_output widget in real time.

            backup_stdout = sys.stdout
            sys.stdout = OutputCatcher()
            if gen_command["program"] == "RDKit":
                self.parent.shell_output.append("Running RDKit...")
                self.parent.shell_output.append("Please wait...")
                try:
                    ...
                    self.parent.failure(f"An error occurred while running AQME: {str(e)}")
                    logging.error(f"Error running AQME: {str(e)}")
                    self.parent.shell_output.append(f"Error: {str(e)}")
                    self.parent.shell_output.append("AQME run failed.")
                    self.parent.shell_output.append("Please check the input parameters and try again.")
                finally:
                    sys.stdout = backup_stdout
            elif gen_command["program"] == "CREST":
                self.parent.shell_output.append("Running CREST...")
                self.parent.shell_output.append("Please wait...")
                try:
                    print("Preparing to run AQME...")
                    print("Running AQME...")
